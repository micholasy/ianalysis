...
{
    $mysql = new SaeMysql();
    $sql = "UPDATE `app_ianalysis`.`weibo_relation` SET `
    	friends` = \"{$mysql->escape($friends)}\",`followers` = \"{$mysql->escape($followers)}\",`bi_followers` = \"{$mysql->escape($bi_followers)}\" WHERE `weibo_relation`.`uid` = \"{$uid}\";";      
    $mysql->runSql( $sql );
    if( $mysql->errno() != 0)
    {
        die( "Error:" . $mysql->errmsg() );
